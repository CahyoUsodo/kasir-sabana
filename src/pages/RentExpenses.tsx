...
export default function RentExpensesPage() {
  const { can } = useAuth();
  ...
  if (!can('manage_stock_inout')) {
    return <LockedPage title="Pengeluaran Sewa Ruko" permissionLabel="Kelola Transaksi & Stok" />;
  }
  ...
  const submitDailyExpense = async () => {
    try {
      ...
      toast.success('Pengeluaran sewa ruko berhasil disimpan');
    } catch (error) {
      console.error(error);
      toast.error(error instanceof Error ? error.message : 'Gagal menyimpan pengeluaran sewa ruko');
    }
  };
  ...
  const removeDailyExpense = async (expenseId: number) => {
    try {
      ...
      toast.success('Pengeluaran sewa ruko dihapus');
    } catch (error) {
      console.error(error);
      toast.error('Gagal menghapus pengeluaran sewa ruko');
    }
  };
  ...
  return (
    <div className="px-4 pt-6 pb-20 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold">Pengeluaran Sewa Ruko</h1>
          <p className="text-xs text-muted-foreground mt-1">
            Catat pengeluaran sewa ruko cabang.
          </p>
        </div>
      </div>
        ...
        <Card className="border-0 shadow-sm overflow-hidden">
          <CardHeader className="p-4 pb-3">
            <CardTitle className="text-sm flex items-center gap-1.5">
              ...
              Pencatatan Sewa Ruko
            </CardTitle>
            <CardDescription className="text-xs">
              Untuk mencatat pembayaran sewa ruko, biaya listrik/air ruko, atau pengeluaran sewa lainnya.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-4 pt-0 space-y-3">
            <div className="grid gap-3 sm:grid-cols-[160px_1fr]">
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold">Nominal Sewa (Rp)</Label>
                <Input
                  type="text"
                  inputMode="numeric"
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold">Detail Sewa</Label>
                <Input
                  placeholder="Contoh: Sewa Ruko Bulan Juni"
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold">Tanggal Pembayaran Sewa</Label>
              <Input
                type="date"
                value={expenseDate}
            ...
            <div className="rounded-xl border bg-muted/20 p-3 space-y-2">
              ...
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-muted-foreground">
                  Total pengeluaran sewa {expenseHistoryDate ? format(new Date(`${expenseHistoryDate}T12:00:00`), 'dd MMM yyyy', { locale: localeId }) : 'tanggal dipilih'}
                </span>
              ...
              <div className="space-y-2">
                {filteredDailyExpenses.length === 0 ? (
                  <p className="text-[11px] text-muted-foreground">Belum ada pengeluaran sewa ruko pada tanggal yang dipilih.</p>
                ) : (
